...
class AbcThunkGen:
    ...
    def emit_h(self, out, name):
        ...
        out.println("/* clang (and gcc) knows about the compiler builtin _Pragma, Visual Studio 2008 C++ compiler does not */");
        out.println("#ifdef __clang__");
        out.println("   #define _PRAGMAFUNCTION(...) _Pragma(__VA_ARGS__)");
        out.println("#else");
        out.println("   #define _PRAGMAFUNCTION(...) ");
        out.println("#endif");
        out.println('')
    ...
    def emit_cpp(self, out, name):
        ...
        if opts.externmethodandclassetables:
            out.println("");
            out.println('const NativeClassInfo* '+name+"_classEntriesExtern = "+name+"_classEntries;");
            out.println('const NativeMethodInfo* '+name+"_methodEntriesExtern = "+name+"_methodEntries;");
                ...
                for i in range(0, mi.optional_count+1):
                    ...
                    if t.is_interface:
                        ...
                        if mi.kind == TRAIT_Setter:
                            idx += 1
                        doubleQuoteChar = '"'
                        arrayBoundsDiag1 = "_PRAGMAFUNCTION(" + doubleQuoteChar + "clang diagnostic push" + doubleQuoteChar + ")"
                        arrayBoundsDiag2 = "_PRAGMAFUNCTION(" + doubleQuoteChar + "clang diagnostic ignored \\" + doubleQuoteChar + "-Warray-bounds\\" + doubleQuoteChar + doubleQuoteChar + ")"
                        arrayBoundsDiag3 = "_PRAGMAFUNCTION(" + doubleQuoteChar + "clang diagnostic pop" + doubleQuoteChar + ")"
                        out.println("%s" % arrayBoundsDiag1)
                        out.println("%s" % arrayBoundsDiag2)
                        out.println("avmplus::MethodEnv* const method = vtable->methods[%d];" % idx)
                        out.println("%s" % arrayBoundsDiag3)
                        if argc == 0:
